...
# MySQL depuis Kubernetes (pour les microservices déployés sur K8s)
resource "aws_security_group_rule" "db_from_kubernetes" {
  type                     = "ingress"
  from_port                = 3306
  to_port                  = 3306
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.tf_kubernetes.id
  security_group_id        = aws_security_group.tf_db.id
  description              = "MySQL from Kubernetes pods"
}
...
# ❌ SUPPRIMÉ : jenkins_to_backend (doublon de backend_from_jenkins)
